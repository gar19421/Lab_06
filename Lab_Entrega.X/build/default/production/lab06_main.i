...
; CONFIG1
  CONFIG FOSC = INTRC_NOCLKOUT ; Oscillator Selection bits (RC oscillator: CLKOUT function on ((PORTA) and 07Fh), 6/OSC2/CLKOUT pin, RC on ((PORTA) and 07Fh), 7/OSC1/CLKIN)
...
reiniciar_tmr2 macro ; reiniciar de Timer2
    ...
    bcf ((PIR1) and 07Fh), 1 ;limpiar bandera de interrupción timer2
    endm
;-------------------------------------------------------------------------------
; Variables
;-------------------------------------------------------------------------------
...
isr: ; rutina de interrupción
    btfsc ((INTCON) and 07Fh), 2 ; verifica si la bandera de interrupcion tmr0 esta levantada
    call int_tmr0 ;subrutina de interrupción de timer0
    btfsc ((PIR1) and 07Fh), 0; verifica si la bandera de interrupcion tmr1 esta levantada
...
;-------------------------------------------------------------------------------
main:
    call config_io ; PORTA salida; ((PORTB) and 07Fh), 7 y ((PORTB) and 07Fh), 2 como input
...
config_reloj: ; Configuración de reloj interno
    ...
    bcf ((OSCCON) and 07Fh), 4 ; Configuración del oscilador a 1MHz
    bsf ((OSCCON) and 07Fh), 0 ; Seleccionar el reloj interno
    return
...
config_tmr0:
    banksel TRISA
    bcf ((OPTION_REG) and 07Fh), 5 ;habilitar reloj interno
...
config_tmr1:
    ...
    bcf ((T1CON) and 07Fh), 1 ; Seleccion del reloj interno
    bsf ((T1CON) and 07Fh), 5
    bsf ((T1CON) and 07Fh), 4 ; PS a 1:8
    reiniciar_tmr1
    return
...
config_tmr2:
    banksel PORTA
    bsf ((T2CON) and 07Fh), 2 ; reloj interno

    bsf ((T2CON) and 07Fh), 6
    bsf ((T2CON) and 07Fh), 5
    bsf ((T2CON) and 07Fh), 4
    bsf ((T2CON) and 07Fh), 3;POSTCALEER (1:16)

    bsf ((T2CON) and 07Fh), 1
    bsf ((T2CON) and 07Fh), 3;PS (1:16)
